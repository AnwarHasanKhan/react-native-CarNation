...
import { MONTHLY_PLANS } from '../../model/MonthlyPlans';
...
const MonthlyPackageCard = ({ pkg, isActive, onPress }) => (
  <TouchableOpacity
    activeOpacity={0.8}
    onPress={onPress}
...
export default function Subscription({ navigation }) {
  ...
  const [selectedId, setSelectedId] = useState('cm_basic');

  const MonthlyPlans = MONTHLY_PLANS[vehicle];
  const selected =
    MonthlyPlans.find(p => p.id === selectedId) ?? MonthlyPlans[0];
  console.log('Console from Subs:', selected);

  const handleVehicleChange = key => {
    setVehicle(key);
    setSelectedId(MONTHLY_PLANS[key][0].id);
  };
